...
for training_columns in TRAINING_COLUMNS:
    analysis = GeneralizedAnalysis(params['NBODY'], mc_path, data_path,
                                   ...
                                   dedicated_background=0, training_columns=training_columns)
...
for split_type in split_list:
    ...
    if split_type == 'matter':
        signal_selection = '{}<=HypCandPt<={} and ArmenterosAlpha > 0'.format(
            ...
            for ctbin in zip(params['CT_BINS'][:-1], params['CT_BINS'][1:]):
                ...
                if args.train:
                    ...
                    analysis.save_model(model, ct_range=ctbin, cent_class=cclass,
                                        pt_range=ptbin, split_string=split_string)
                    if (args.significance == False):
                        break
                else:
                    model = analysis.load_model(ct_range=ctbin, cent_class=cclass,
                                                pt_range=ptbin, split_string=split_string)
