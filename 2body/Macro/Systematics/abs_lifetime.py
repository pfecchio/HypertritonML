    ...
    for cclass in params['CENTRALITY_CLASS']:
        ...
        for model in bkgModels:
            ...
            for iBin in range(1, h1RawCounts.GetNbinsX()+1):
                ...
                h1RawCounts.SetBinContent(iBin, h2RawCounts.GetBinContent(1, iBin) / h1PreselEff.GetBinContent(iBin) / ranges['BEST'][iBin-1] / h1RawCounts.GetBinWidth(iBin)/(1-absorp_hist.GetBinContent(iBin)))
                h1RawCounts.SetBinError(iBin, h2RawCounts.GetBinError(1, iBin) / h1PreselEff.GetBinContent(iBin) / ranges['BEST'][iBin-1] / h1RawCounts.GetBinWidth(iBin)/(1-absorp_hist.GetBinContent(iBin)))
                raws.append([])
                errs.append([])
                ...
                for eff in np.arange(ranges['SCAN'][iBin-1][0], ranges['SCAN'][iBin-1][1], ranges['SCAN'][iBin-1][2]):
                    ...
                    raws[iBin-1].append(h2RawCounts.GetBinContent(1,iBin) / h1PreselEff.GetBinContent(iBin) / eff / h1RawCounts.GetBinWidth(iBin)/(1-absorp_hist.GetBinContent(iBin)))
                    errs[iBin-1].append(h2RawCounts.GetBinError(1,iBin) / h1PreselEff.GetBinContent(iBin) / eff / h1RawCounts.GetBinWidth(iBin)/(1-absorp_hist.GetBinContent(iBin)))
...
for name, func in zip(abs_names_list, fit_func_list):
    string = '#tau = {:.1f} #pm {:.1f} ps '.format(func.GetParameter(1), func.GetParError(1))
