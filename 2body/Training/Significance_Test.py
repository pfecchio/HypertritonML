...
import os
from ROOT import TF1,TFile,gDirectory,vector,gROOT
...
def Yield(pt_bin,Centrality_bin):
    ...
    # if Centrality_bin==[0,90]:
    #     pT_yield = 0
    #     BlastWave0=bwFile.Get("BlastWave/BlastWave0") 
    #     BlastWave1=bwFile.Get("BlastWave/BlastWave1")
    #     BlastWave2=bwFile.Get("BlastWave/BlastWave2") 
    #     Integral0 = BlastWave0.Integral(0,10,1e-8)
    #     pT_yield=pT_yield+2**Scale_Factor[0]*BlastWave0.Integral(pt_bin[0],pt_bin[1],1e-8)/(pt_bin[1]-pt_bin[0])/Integral0
    #     Integral1 = BlastWave1.Integral(0,10,1e-8)
    #     pT_yield=pT_yield+2**Scale_Factor[1]*BlastWave1.Integral(pt_bin[0],pt_bin[1],1e-8)/(pt_bin[1]-pt_bin[0])/Integral1
    #     Integral2 = BlastWave2.Integral(0,10,1e-8)
    #     pT_yield=pT_yield+2**Scale_Factor[2]*BlastWave2.Integral(pt_bin[0],pt_bin[1],1e-8)/(pt_bin[1]-pt_bin[0])/Integral2
...
def SignificanceScan(df,ct_cut,pt_cut,centrality_cut,efficiency_array,eff_pres,n_ev,custom=False,draw=False):    
    ...
    for i in score_list:
        ...
        if centrality_cut ==[0,90]:
            YpTt = YpTt + ExpectedSignal(efficiency_array[index],pt_cut,n_ev[0],eff_pres,[0,10])
            YpTt = YpTt + ExpectedSignal(efficiency_array[index],pt_cut,n_ev[1],eff_pres,[10,30])
            YpTt = YpTt + ExpectedSignal(efficiency_array[index],pt_cut,n_ev[2],eff_pres,[30,50])
            YpTt = YpTt + ExpectedSignal(efficiency_array[index],pt_cut,n_ev[3],eff_pres,[50,90])
        else:
    ...
    if not os.path.exists(os.environ['HYPERML_FIGURES']+'/SigTimesEff/'):
        os.makedirs(os.environ['HYPERML_FIGURES']+'/SigTimesEff/')
    ...
    for index in range(0,2):
        ...
        if index==1:
            ...
            axs[0].grid()
            filename = 'SigTimesEff_Ct_{:.2f}_{:.2f}_pT_{:.2f}_{:.2f}_Cen_{:.2f}_{:.2f}.pdf'.format(ct_cut[0],ct_cut[1],pt_cut[0],pt_cut[1],centrality_cut[0],centrality_cut[1])
        ...
        if index is 1:
            plt.savefig(os.environ['HYPERML_FIGURES']+'/SigTimesEff/'+filename)
        else:
